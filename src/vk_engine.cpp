...
void VulkanEngine::draw()
{
    ...
    //now that the commands are finished executing, reset command buffer to begin recording
    VK_CHECK(vkResetCommandBuffer(cmd, 0));

    //begin the command buffer recording, we will use this command buffer exactly once so we let vulkan know that
    ...
    // transition our main draw image into general layout so we can write int oit
    // undefined/ don't care old layout
    vkutil::transition_image(cmd, _drawImage.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL);
    // now we no longer do the clear on the swapchain image-- we just do it on the drawImage
    draw_background(cmd);
    ...
    // wait on present semaphore, it's signaled when the swapchain is ready
    ...
    // render fence will now block until the commands finish executing
...
void VulkanEngine::draw_background(VkCommandBuffer cmd)
{
    ...
    //NOTE: VK IMAGE LAYOUT GENERAL -- not best for rendering, but we can use one for reading or rasterization commands in the future
    ...
    //execute compute dispact pipeline
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd)
{
    ...
    //camera projection
    glm::mat4 projection = glm::perspective(glm::radians(70.0f), (float)_drawExtent.width / (float)_drawExtent.height, 0.1f, 10000.0f);
    //invert y direction because it's broken
    projection[1][1] *= -1;
...
void VulkanEngine::init_vulkan()
{
    ...
    // 1: Building the instance and debug messenger-- instance allows us to interface with drivers
    ...
    // 2: Creating the surface that we draw to
    SDL_Vulkan_CreateSurface(_window, _instance, &_surface);
    // Enable Dynamic rendering and better sync structures
    VkPhysicalDeviceVulkan13Features feature{ .sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_3_FEATURES };
    feature.dynamicRendering = true;
    feature.synchronization2 = true;
    //Enable buffer device addresses and descriptor indexing
    ...
    vkb::PhysicalDevice physicalDevice = selector
        ...
        .set_required_features_13(feature)
    ...
    //this is where the commands for drawing get submitted
...
void VulkanEngine::init_swapchain()
{
    ...
    //build image view for the draw image to use
...
void VulkanEngine::init_descriptors()
{
    ...
    //making the descriptor set layout for compute draw calls
    {
    ...
    _drawImageDescriptors = globalDescriptorAllocator.Allocate(_device, _drawImageDescriptorLayout, nullptr);
...
void VulkanEngine::init_background_pipeline()
{
    ...
    //not really explored-- used to send data and set values in shaders 
    //without creating buffers or modifying and binding descriptor sets
...
void VulkanEngine::init_mesh_pipeline()
{
    ...
    //push constants like world matrix and such
...
{
    //1. Creating the descriptor pool for imgui
    //oversized pool-- we can optimize this later
    ...
    // add destruction of imgui created structs
    _mainDeletionQueue.push_function([=]()
        {
...
GPUMeshBuffers VulkanEngine::uploadMesh(std::span<uint32_t> indices, std::span<Vertex> vertices)
{
    ...
    //common pattern in vulkan -- allocate cpu memory to stage and write data to the gpu buffers
...
AllocatedImage VulkanEngine::create_image(VkExtent3D size, VkFormat format, VkImageUsageFlags usage, bool mipmapped)
{
    ...
    //allocating images based on dedicated GPU memory
